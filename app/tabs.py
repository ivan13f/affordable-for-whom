...
rents_BEZ = load_csv('../data/csv/rent/rents_BEZ.csv')
rents_PLR = load_csv('../data/csv/rent/rents_PLR.csv')
rent_structure = load_csv('../data/csv/rent/rent_price_structure_2023.csv')

income_households_berlin= load_csv('../data/csv/income/income_household_13_23.csv')
income_persons_berlin  = load_csv('../data/csv/income/disposable_income.csv')

wohnatlas_2022  = load_csv('../data/csv/wohnatlas/wohnatlas_2022.csv')

households  = load_csv('../data/csv/general_housing/households_berlin.csv')

@st.cache_data
def load_outlook_data():
    rents_PLR = pd.read_csv("../data/csv/rent/rents_PLR.csv")
    rents_PLR = rents_PLR[rents_PLR["year"] == 2023]

    geojson = gpd.read_file("../data/geo/2021_PLR.geojson").to_crs(epsg=4326)
    rents_PLR["plr_id"] = rents_PLR["plr_id"].astype(str).str.zfill(8)
    geojson["PLR_ID"] = geojson["PLR_ID"].astype(str)

    merged = geojson.merge(rents_PLR, left_on="PLR_ID", right_on="plr_id")
...
def show_intro_tab():
    ...
    with col1:
        ...
        geojson_rents_plr = json.loads(gdf_rents_filtered.drop_duplicates("plr_id")[["plr_id", "geometry"]].to_json())
...
def show_demand_tab():
    ...
    pop_BEZ= load_csv('../data/csv/population/population_BEZ.csv')
    net_migration= load_csv('../data/csv/population/net_migration_BEZ.csv')
...
def show_social_tab():
    ...
    subsidies = load_csv('../data/csv/public_housing/social_housing_subsidized_berlin.csv')
    sh_subsidized_totals = load_csv('../data/csv/public_housing/sh_subsidized_totals.csv')
    wbs_berlin  = load_csv('../data/csv/public_housing/wbs_berlin.csv')
...
def show_outlook_tab():
    ...
    with col1:
        st.markdown("""### <span style='display: inline-flex; align-items: center; justify-content: center; width: 40px; height: 40px; border-radius: 50%; background: #D4583B; color: white; font-weight: bold; margin-right: 40px; font-size: 20px;'>1</span> """, unsafe_allow_html=True)
    with col2:
    ...
    col1, col2= st.columns([1,24])
    with col1:
        st.markdown("""### <span style='display: inline-flex; align-items: center; justify-content: center; width: 40px; height: 40px; border-radius: 50%; background: #D4583B; color: white; font-weight: bold; margin-right: 40px; font-size: 20px;'>2</span> """, unsafe_allow_html=True)
    with col2:
    ...
    col1, col2= st.columns([1,24])
    with col1:
        st.markdown("""### <span style='display: inline-flex; align-items: center; justify-content: center; width: 40px; height: 40px; border-radius: 50%; background: #D4583B; color: white; font-weight: bold; margin-right: 40px; font-size: 20px;'>3</span> """, unsafe_allow_html=True)
    with col2:
    ...
    col1, col2= st.columns([1,24])
    with col1:
        st.markdown("""### <span style='display: inline-flex; align-items: center; justify-content: center; width: 40px; height: 40px; border-radius: 50%; background: #D4583B; color: white; font-weight: bold; margin-right: 40px; font-size: 20px;'>4</span> """, unsafe_allow_html=True)
    with col2:
    ...
    col1, col2= st.columns([1,24])
    with col1:
        st.markdown("""### <span style='display: inline-flex; align-items: center; justify-content: center; width: 40px; height: 40px; border-radius: 50%; background: #D4583B; color: white; font-weight: bold; margin-right: 40px; font-size: 20px;'>5</span> """, unsafe_allow_html=True)
    with col2:
    ...
    with st.expander("What lays outside of the scope of this analysis?", expanded=True):
        st.markdown("""
    ...
    with col1:
        ...
        merged, geojson = load_outlook_data()
    ...
    with col2:
        ...
        your_burden = go.Figure(go.Choroplethmapbox(
        ...
        st.plotly_chart(your_burden, use_container_width=True)
